...
void handleLogin(SOCKET connectedSocket, std::string body, Account& client, int indexClient, int thread) {
		...
		for (int i = 0; i < size; i++) {
			if (accounts[i].username == body) {
				if (!accounts[i].status) {
					...
					clients[thread][indexClient].acc.username = body;
...
void handleBye(SOCKET connectedSocket, Account& client, int indexClient, int thread) {
	if (client.username == "") {
		handleSendData(connectedSocket, NOT_LOGGED_IN);
	}
	else {
		...
		for (int i = 0; i < size; i++) {
			if (accountsT[i].username == client.username) {
				...
				client.username = "";
				EnterCriticalSection(&section[thread]);
				clients[thread][indexClient].acc = client;
				LeaveCriticalSection(&section[thread]);
				return;
			}
		}
...
void handleDisconnect(Account& client, int indexClient, int thread) {
	int size = accounts.size();
	for (int i = 0; i < size; i++) {
